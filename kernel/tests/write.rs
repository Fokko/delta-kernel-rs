...
use test_utils::{
    ...
    remove_all_and_get_remove_actions, remove_scan_files_with_selection, resolve_field,
    set_json_value, setup_test_tables, test_read, test_table_setup, test_table_setup_mt,
...
async fn test_remove_files_manifest_commit_mode() -> Result<(), Box<dyn std::error::Error>> {
    // Verify that remove_files in manifest commit mode is rejected when no checkpoint action
    // exists.
