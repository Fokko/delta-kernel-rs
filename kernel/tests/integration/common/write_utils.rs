...
/// Validate that `commitInfo["snapshotId"]` is a positive integer and remove it so that
/// callers can compare the remaining fields without hard-coding a non-deterministic value.
pub fn validate_and_strip_snapshot_id(commit_info: &mut serde_json::Value) {
    let snapshot_id = commit_info
        .get("snapshotId")
        .and_then(|v| v.as_i64())
        .expect("snapshotId should be present in commitInfo and be an integer");
    assert!(
        snapshot_id > 0,
        "snapshotId should be positive, got {snapshot_id}"
    );
...
/// Removes all files returned by `scan` from `txn`. Returns the total count of files removed.
pub fn remove_all_scan_files(
    txn: &mut Transaction,
    scan: Scan,
    engine: &dyn Engine,
) -> DeltaResult<usize> {
...
/// Write two small parquet files to the table using a manifest commit and verify commit stats.
pub async fn batch_write_data_and_check_result_and_stats(
    table_url: Url,
    schema: SchemaRef,
    engine: Arc<DefaultEngine<TokioBackgroundExecutor>>,
    expected_since_commit: u64,
