...
use crate::common::write_utils::{
    ...
    validate_and_strip_snapshot_id, validate_txn_id, write_data_and_check_result_and_stats,
    ZERO_UUID,
};
...
async fn test_append() -> Result<(), Box<dyn std::error::Error>> {
        ...
        setup_test_tables(schema.clone(), &[], None, "test_table").await?
    {
        ...
        // validate snapshotId is a positive integer and strip it for stable comparison
        validate_and_strip_snapshot_id(&mut parsed_commits[0]["commitInfo"]);
