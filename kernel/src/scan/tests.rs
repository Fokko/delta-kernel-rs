...
pub(crate) fn get_files_for_scan_allow_dvs(
    ...
    fn scan_metadata_callback(paths: &mut Vec<String>, scan_file: ScanFile) {
        paths.push(scan_file.path.to_string());
        // Note: scan_file.dv_info.deletion_vector may be Some when scanning from content root with
        // leaf manifests
    }
    let mut files = vec![];
    for res in scan_metadata_iter {
...
fn test_replay_for_scan_metadata() {
    ...
    let action_and_snapshot_info = scan.replay_for_scan_metadata(&engine).unwrap();

    let data: Vec<_> = action_and_snapshot_info.actions.try_collect().unwrap();
...
{{"checkpoint":{{"version":{},"contentRoot":{{"path":"{}","sizeInBytes":1024}}{checkpoint_pm}}}}}"#,
    ...
    let action_and_snapshot_info = scan.replay_for_scan_metadata(engine.as_ref())?;
    let action_batches: Vec<_> = action_and_snapshot_info.actions.try_collect()?;
...
impl ParquetHandler for EmptyParquetHandler {
    ...
    fn write_parquet_file(
        ...
        _write_config: &crate::ParquetWriterConfig,
    ) -> crate::DeltaResult<crate::ParquetWriteResult> {
        unimplemented!()
    }
}
