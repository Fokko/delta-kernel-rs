...
impl Snapshot {
    ...
    fn try_new_from(
        ...
        // Replay only new commits (> existing_snapshot_version) for P&M and checkpoint action.
        // Use segment_after_crc to strip the checkpoint and old commit files, ensuring the replay
        // doesn't regress to stale checkpoint P&M. Also skip a stale CRC (at or below
        // existing_snapshot_version): if no P&M is found in the pruned replay,
        // read_protocol_metadata_opt would fall back to the CRC's P&M, which could predate
        // changes already in the existing snapshot.
        let existing_protocol = existing_snapshot.table_configuration().protocol();
        let (new_metadata, new_protocol, new_checkpoint_action) = {
            ...
            // segment_after_crc strips the checkpoint and filters to commits >
            // existing_snapshot_version, preventing the replay from picking up stale
            // P&M from the checkpoint.
            new_log_segment
                .segment_after_crc(existing_snapshot_version)
                .protocol_and_metadata_and_checkpoint(engine, Some(existing_protocol), crc)?
        };
        ...
        let combined_log_segment = LogSegment::try_new(
            ...
            log_root,
            requested_version,
            // Use the new checkpoint schema hint if found, otherwise preserve the existing one.
            new_checkpoint_schema
                .and_then(|s| s.schema)
                .or_else(|| existing_log_segment.get_checkpoint_schema()),
        )?;
