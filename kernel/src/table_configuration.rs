...
impl TableConfiguration {
    ...
    pub(crate) fn partition_values_schema(&self) -> DeltaResult<Option<SchemaRef>> {
        ...
        let fields: Vec<crate::schema::StructField> = partition_columns
            .iter()
            .filter_map(|col_name| {
                self.logical_schema().field(col_name).map(|field| {
...
mod test {
    ...
    fn create_unknown_feature_config(
        ...
        let tc = TableConfiguration::try_new(metadata, protocol, None, table_root, 0).unwrap();
    ...
    fn test_iceberg_native_v4_requires_domain_metadata() {
        ...
        assert_result_error_with_message(
            config.ensure_operation_supported(Operation::Write),
            "Feature 'icebergNativeV4-experimental' requires 'domainMetadata' to be supported",
        );
    }
    ...
    fn test_iceberg_native_v4_requires_metadata_tree() {
        ...
        assert_result_error_with_message(
            config.ensure_operation_supported(Operation::Write),
            "Feature 'icebergNativeV4-experimental' requires 'metadataTree-experimental' to be supported",
        );
    }
