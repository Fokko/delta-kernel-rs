...
//! [`CreateTableTransaction`](super::super::create_table::CreateTableTransaction) from
//! user-provided schema, properties, and data layout options.
//!
...
use crate::table_properties::{
    ...
    ENABLE_ICEBERG_COMPAT_V2, ENABLE_ICEBERG_COMPAT_V3, ENABLE_ICEBERG_NATIVE_V4_EXPERIMENTAL,
...
type CreateTableTransaction = Transaction<CreateTable>;
...
///
/// Use this to configure table properties before building a
/// [`CreateTableTransaction`](super::super::create_table::CreateTableTransaction). If the table
/// build fails, no transaction will be created.
///
...
impl CreateTableTransactionBuilder {
    ...
    /// Builds a [`CreateTableTransaction`](super::super::create_table::CreateTableTransaction) that
    /// can be committed to create the table.
    ///
    /// The returned [`CreateTableTransaction`](super::super::create_table::CreateTableTransaction)
    /// only exposes operations that are valid for table creation. Operations like removing
    /// files, removing domain metadata, or updating deletion vectors are not available,
    ...
    pub fn build(
        ...
        // Build TableConfiguration for the pre-commit state of the new table.
