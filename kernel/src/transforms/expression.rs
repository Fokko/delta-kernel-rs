...
use crate::expressions::{
    ...
    MapToStructExpression, OpaqueExpression, OpaquePredicate, ParseJsonExpression,
    ParsePartitionValuesExpression, PartitionValuesToMapExpression, Predicate, Scalar, Transform,
...
pub trait ExpressionTransform<'a> {
    ...
    ) -> Self::Output<ParsePartitionValuesExpression> {
    ...
    fn transform_expr(&mut self, expr: &'a Expression) -> Self::Output<Expression> {
        match expr {
            ...
            Expression::ParsePartitionValues(p) => {
                let child = self.transform_expr_parse_partition_values(p);
                map_owned_or_else(expr, child, Expression::ParsePartitionValues)
            }
            Expression::PartitionValuesToMap(p) => {
                let child = self.transform_expr_partition_values_to_map(p);
                map_owned_or_else(expr, child, Expression::PartitionValuesToMap)
            }
    ...
    /// schema is not transformed.
    fn recurse_into_expr_parse_partition_values(
        &mut self,
        expr: &'a ParsePartitionValuesExpression,
    ) -> Self::Output<ParsePartitionValuesExpression> {
        let f =
            |map_expr| ParsePartitionValuesExpression::new(map_expr, expr.output_schema.clone());
        map_owned_or_else(expr, self.transform_expr(&expr.map_expr), f)
    }

    /// Recursively transforms the child expression of a [`PartitionValuesToMapExpression`].
