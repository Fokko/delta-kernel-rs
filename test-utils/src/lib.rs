...
pub async fn read_manifest_parquet(url: &Url) -> Result<RecordBatch, Box<dyn std::error::Error>> {
    ...
    let file_path = delta_kernel::object_store::path::Path::from_url_path(url.path())?;
...
/// Type alias for deletion vector tuple: (storage_type, path_or_inline_dv, offset, size_in_bytes,
/// cardinality)
type DeletionVectorTuple = (String, String, Option<i32>, i32, i64);

/// Type alias for add file with deletion vector: (file_path, file_size, mod_time, num_records,
/// deletion_vector)
